                ...
                <div className="bg-gray-900 p-6 rounded-lg border border-green-600">
                  ...
                  <div className="bg-gray-800 p-3 rounded-lg">
                    <p className="text-sm text-gray-300">
                      <strong>Expected Outcome:</strong> {recommendations.primary_recommendation.expected_outcome} additional publications
                    </p>
                  </div>
                </div>

                {/* Balance Recommendation */}
                {recommendations.balance_recommendation && (
                  <div className="bg-gray-900 p-6 rounded-lg border border-blue-600">
                    <div className="flex items-center gap-2 mb-4">
                      <Badge variant="outline" className="bg-blue-100 text-blue-800">
                        Balance Recommendation
                      </Badge>
                      <span className="text-sm text-gray-400">Portfolio Optimization</span>
                    </div>
                    
                    <h3 className="text-xl font-semibold text-blue-400 mb-3">
                      {recommendations.balance_recommendation.domain}
                    </h3>
                    
                    <p className="text-gray-300 mb-4">
                      {recommendations.balance_recommendation.action}
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          <span className="text-gray-400">Current Studies:</span>
                          <span className="font-medium text-white">
                            {recommendations.balance_recommendation.current_studies}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-400">Suggested Decrease:</span>
                          <span className="font-medium text-red-500">
                            -{recommendations.balance_recommendation.suggested_decrease}%
                          </span>
                        </div>
                      </div>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-green-500 flex items-center gap-1">
                            <DollarSign className="h-3 w-3" />
                            {recommendations.balance_recommendation.cost_savings?.toLocaleString() || 'N/A'}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-400">Risk Level:</span>
                          <span className="font-medium text-yellow-500">
                            {recommendations.balance_recommendation.risk_level || 'Medium'}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="text-center">
                        <div className="text-2xl font-bold text-green-400">
                          {recommendations.financial_impact.net_impact > 0 ? '+' : ''}
                          ${recommendations.financial_impact.net_impact?.toLocaleString() || 'N/A'}
                        </div>
                        <div className="text-sm text-gray-400">Net Impact</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-blue-400">
                          {recommendations.financial_impact.total_investment?.toLocaleString() || 'N/A'}
                        </div>
                        <div className="text-sm text-gray-400">Total Investment</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-purple-400">
                          {recommendations.financial_impact.expected_roi || 'N/A'}%
                        </div>
